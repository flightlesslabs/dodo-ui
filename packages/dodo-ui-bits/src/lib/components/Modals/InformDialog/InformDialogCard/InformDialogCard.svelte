<script lang="ts" module>
  export type InformDialogCardProps = CardProps & {
    ...
    acceptButtonProps?: ModalAcceptButtonProps;
...
<script lang="ts">
  ...
  import ModalAcceptButton, {
    type ModalAcceptButtonProps,
  } from '../../ButtonControls/ModalAcceptButton.svelte';
...
<ModalCard {...restProps} class={classes.join(' ')}>
  {#snippet controls()}
    ...
    {:else}
      <ModalAcceptButton {...acceptButtonProps} onclick={handleOnAccept} />
    {/if}
  {/snippet}
</ModalCard>
